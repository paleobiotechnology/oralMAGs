...
fix_labels <- fread("05-results/methanobrevibacter_metadata.tsv") %>%
  rename(Label = Bin) %>%
  rename(Bin = prepublication_bin) %>%
  relocate(Bin, .before = Label) %>%
  select(Label, Bin) %>%
...
heatmap_metadata <- ggplot(methanobrevibacter_metadata2, aes(x = category, y = Label, fill = Geo_Cluster)) +
  geom_tile() +
  theme_void() +
...
methanobrevibacter_cas_input <- methanobrevibacter_cas_input %>%
  inner_join(., metadata %>%
               select(Label, Bin), by = "Bin")

heatmap_crispr <- ggplot(methanobrevibacter_cas_input, aes(x = category, y = Label, fill = crispr_metadata)) +
  geom_tile() +
  theme_void() +
  theme(axis.title.x = element_blank(),
        axis.text.x = element_blank(),
...
prophage_heatmap <- ggplot(methanobrevibacter_prophages, aes(x = "Prophage", y = Label, fill = Prophage)) +
  geom_tile() +
  theme_void() +
  theme(axis.title.x=element_blank(),
        axis.text.x=element_blank(),
