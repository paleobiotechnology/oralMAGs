---
title: "Ca. Methanocatella senecta (sp_cl_1189) Tree and Metadata"
author: "Keri Burge"
...
```{r}
tre_methanocatella_c1189_scc <- ape::read.tree("05-results/methanocatella_c1189_scc.raxml.support")

methanocatella_c1189_mad_root <- mad(tre_methanocatella_c1189_scc, 'newick')

methanocatella_c1189_mad_root %>%
  as.list() %>%
  fwrite(., "05-results/methanocatella_c1189_mad_root.nwk", quote = F)
```

```{r}
#Find the root
methanocatella_c1189_mad_root <- mad(tre_methanocatella_c1189_scc, 'full')

#ERS6256706_26.0.refbinned

```


```{r}
methanocatella_c1189_mad_root <- ape::read.tree("05-results/methanocatella_c1189_mad_root.nwk")

methanocatella_c1189_mad_root <- root(methanocatella_c1189_mad_root, outgroup = "ERS6256706_26.0.refbinned")

methanocatella_c1189_mad_root$tip.label <- methanocatella_c1189_mad_root$tip.label %>%
  as.data.frame() %>%
  dplyr::rename(Bin = 1) %>%
...
# create tree
methanocatella_tree_c1189 <- ggtree(methanocatella_c1189_mad_root, ladderize = T)
...
methanocatella_tree_c1189_age <- methanocatella_tree_c1189 %<+% metadata + 
  ...
  scale_color_viridis_c(option = "mako", name = "Age (BP) (log10)", direction = -1, limits = c(0.845097, 5.008601)) +
...
```{r}
#fastBAPS cluster
fastBAPS <- fread("05-results/fastBAPS_methanocatella_cluster1189_snp_counts_and_clusters.tsv") %>%
  mutate(Bin = if_else(str_detect(Bin, "GC|refbinned"), Bin, 
                        str_replace_all(Bin, "-metaspades_00", ".bin.r.") %>%
                        str_replace_all("-metaspades_0", ".bin.r.") %>%
                        str_replace_all("-metaspades", ".bin.r.") %>%
...
### Gene Count
```{r}
scc_gene_list <- fread("05-results/methanocatella_c1189_non_recomb_scc_gene_list.tsv") %>%
  pull()

panaroo <- fread("05-results/methanocatella_c1189_panaroo_gene_presence_absence.Rtab") %>%
...
## Panaroo
```{r}
heatmap_panaroo_data <- fread("05-results/methanocatella_c1189_panaroo_gene_presence_absence.Rtab") %>%
...
gene_abundance <- fread("05-results/methanocatella_c1189_panaroo_gene_presence_absence.Rtab") %>%
