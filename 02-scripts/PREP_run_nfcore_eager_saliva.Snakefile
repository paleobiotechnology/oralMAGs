...
# Step: Run nf-core/eager to process the saliva sequencing data
#
...
#### SAMPLES ###################################################################
samples = pd.read_csv("01-resources/modern_saliva_samples.tsv", sep="\t")
################################################################################

#### Auxilliary functions ######################################################
chemistry = {
    2: ['NextSeq 500', 'Illumina NovaSeq 6000',
...
rule all:
    input:
        "04-analysis/eager/saliva.done"

rule generate_eager_tsv:
    output:
        "04-analysis/eager/saliva.tsv",
    message: "Generate the EAGER input TSV for the saliva samples"
    resources:
        mem_mb = 4000
    threads: 1
    run:
...
rule run_eager:
    input:
        tsv = "04-analysis/eager/saliva.tsv",
        fa = "03-data/refgenomes/hs37ds.fa"
    output:
        touch("04-analysis/eager/saliva.done")
    message: "Run nf-core/EAGER to process the sequencing data of all saliva libraries"
    params:
        outdir = "04-analysis/eager/saliva"
    shell:
        """
