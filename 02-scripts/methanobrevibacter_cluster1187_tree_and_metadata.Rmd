---
title: "Ca. Methanobrevibacter prisca (pr_cl_1187) Tree and Metadata"
author: "Keri Burge"
...
```{r}
tre_methanobrevibacter_c1187_scc <- ape::read.tree("05-results/methanobrevibacter_c1187_scc.raxml.support")

methanobrevibacter_c1187_mad_root <- mad(tre_methanobrevibacter_c1187_scc, 'newick')

methanobrevibacter_c1187_mad_root %>%
  as.list() %>%
  fwrite(., "./05-results/methanobrevibacter_c1187_mad_root.nwk", quote = F)
```

```{r}
#Find the root
methanobrevibacter_c1187_mad_root <- mad(tre_methanobrevibacter_c1187_scc, 'full')

#ERS3774460-megahit_001
```


```{r}
methanobrevibacter_c1187_mad_root <- ape::read.tree("./05-results/methanobrevibacter_c1187_mad_root.nwk")

methanobrevibacter_c1187_mad_rootr <- root(methanobrevibacter_c1187_mad_root, outgroup = "ERS3774460-megahit_001")

methanobrevibacter_c1187_mad_root$tip.label <- methanobrevibacter_c1187_mad_root$tip.label %>%
  as.data.frame() %>%
  dplyr::rename(Bin = 1) %>%
...
# create tree
methanobrevibacter_tree_c1187 <- ggtree(methanobrevibacter_c1187_mad_root, ladderize = T)
...
methanobrevibacter_tree_c1187_age <- methanobrevibacter_tree_c1187 %<+% metadata + 
  ...
  geom_tiplab(size = 1.75, hjust = -0.1)

methanobrevibacter_tree_c1187_age

#ggsave("./05-results/methanobrevibacter_c3_MAD.pdf", plot = methanobrevibacter_tree_c3_age, device = "pdf", scale = 1, width = 6, height = 8, units = c("in"), dpi = 300) 
```
...
```{r}
#fastBAPS cluster
fastBAPS <- fread("05-results/fastBAPS_methanobrevibacter_cluster1187_snp_counts_and_clusters.tsv") %>%
  mutate(Bin = if_else(str_detect(Bin, "GC|refbinned"), Bin, 
                        str_replace_all(Bin, "-metaspades_00", ".bin.r.") %>%
                        str_replace_all("-metaspades_0", ".bin.r.") %>%
                        str_replace_all("-metaspades", ".bin.r.") %>%
...
### Gene Count
```{r}
scc_gene_list <- fread("05-results/methanobrevibacter_c1187_non_recomb_scc_gene_list.tsv") %>%
  pull()

panaroo <- fread("05-results/methanobrevibacter_c1187_panaroo_gene_presence_absence.Rtab") %>%
...
## Panaroo
```{r}
heatmap_panaroo_data <- fread("./05-results/methanobrevibacter_c1187_panaroo_gene_presence_absence.Rtab") %>%
...
gene_abundance <- fread("./05-results/methanobrevibacter_c1187_panaroo_gene_presence_absence.Rtab") %>%
