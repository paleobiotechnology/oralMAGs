---
title: "Ca. Methanocatella cohabitans (pr_cl_1990) Tree and Metadata"
author: "Keri Burge"
...
```{r}
tre_methanocatella_c1190_scc <- ape::read.tree("05-results/methanocatella_c1190_scc.raxml.support")

methanocatella_c1190_mad_root <- mad(tre_methanocatella_c1190_scc, 'newick')

methanocatella_c1190_mad_root %>%
  as.list() %>%
  fwrite(., "./04-analysis/phylogenies/methanocatella/cluster_4_w_CSC/scc_non_recomb/methanocatella_c4_mad_root.nwk", quote = F)
```

```{r}
#Find the root
methanocatella_c1190_mad_root <- mad(tre_methanocatella_c1190_scc, 'full')
...
```{r}
methanocatella_c1190_mad_root <- ape::read.tree("./04-analysis/phylogenies/methanocatella/cluster_4_w_CSC/scc_non_recomb/methanocatella_c4_mad_root.nwk")

methanocatella_c1190_mad_root <- root(methanocatella_c1190_mad_root, outgroup = "ERSX0005001-megahit_029")

methanocatella_c1190_mad_root$tip.label <- methanocatella_c1190_mad_root$tip.label %>%
  as.data.frame() %>%
  dplyr::rename(Bin = 1) %>%
...
# create tree
methanocatella_tree_c1190 <- ggtree(methanocatella_c1190_mad_root, ladderize = T)
...
methanocatella_tree_c1190_age <- methanocatella_tree_c1190 %<+% metadata + 
...
```{r}
#fastBAPS cluster
fastBAPS <- fread("05-results/fastBAPS_methanocatella_cluster4_snp_counts_and_clusters.tsv") %>%
  mutate(Bin = if_else(str_detect(Bin, "GC|refbinned"), Bin, 
                        str_replace_all(Bin, "-metaspades_00", ".bin.r.") %>%
                        str_replace_all("-metaspades_0", ".bin.r.") %>%
                        str_replace_all("-metaspades", ".bin.r.") %>%
...
### Latitude
```{r}
methanocatella_coords <- fread("00-documentation/methanocatella_sample_coords.tsv") %>%
  inner_join(., metadata, by = "Bin")

latitude <- ggplot(methanocatella_coords, aes(x = "latitude", y = Bin, fill = latitude)) +
  geom_tile() +
  theme_void() +
  theme(axis.title.x=element_blank(),
        axis.text.x=element_blank(),
...
### Longitude
```{r}
longitude <- ggplot(methanocatella_coords, aes(x = "longitude", y = Bin, fill = longitude)) +
  geom_tile() +
  theme_void() +
  theme(axis.title.x=element_blank(),
        axis.text.x=element_blank(),
...
### Contig Count
```{r}
methanocatella_stats <- fread("00-documentation/methanocatella_mag_stats.tsv") %>%
  inner_join(., metadata)

contigs <- ggplot(methanocatella_stats, aes(x = "quast_n_contigs", y = Bin, fill = quast_n_contigs)) +
  geom_tile() +
  theme_void() +
  theme(axis.title.x=element_blank(),
        axis.text.x=element_blank(),
...
### Completeness
```{r}
completeness <- ggplot(methanocatella_stats, aes(x = "checkM.completeness", y = Bin, fill = checkM.completeness)) +
  geom_tile() +
  theme_void() +
  theme(axis.title.x=element_blank(),
        axis.text.x=element_blank(),
...
### Gene Count
```{r}
scc_gene_list <- fread("04-analysis/phylogenies/methanocatella/cluster_4_w_CSC/scc_non_recomb/methanocatella_C4_w_CSC_non_recomb_scc_gene_list.tsv") %>%
  pull()

panaroo <- fread("04-analysis/panaroo/methanocatella/w_new_mag/cluster_4/results_cluster_4/gene_presence_absence.Rtab") %>%
...
methanocatella_cas <- fread("./04-analysis/cctyper/methanocatella/methanocatella_cctyper_cas_table.tsv") %>%
...
methanocatella_cas_ranked <- methanocatella_cas %>%
  mutate(priority = crispr_priority[Best_type]) %>%
  arrange(Bin, priority)

crispr_pairs <- methanocatella_cas_ranked %>%
  group_by(Bin) %>%
  summarise(
...
methanocatella_cas_input <- crispr_pairs %>%
  pivot_longer(cols = c(crispr1, crispr2), names_to = "category", values_to = "crispr_metadata")


heatmap_crispr <- ggplot(methanocatella_cas_input, aes(x = category, y = Bin, fill = crispr_metadata)) +
  geom_tile() +
  theme_void() +
  theme(axis.title.x = element_blank(),
        axis.text.x = element_blank(),
...
## CAZyme
```{r}
cazymes_methanocatella <-fread("05-results/methanocatella_dbcan3_cazyme_table.tsv")


cazymes_methanocatella <- cazymes_methanocatella %>%
  inner_join(., metadata, by = "Bin")

cazymes_methanocatella_table <- cazymes_methanocatella %>%
...
cazymes_methanocatella_table2 <- cazymes_methanocatella_table %>%
  unique() %>%
  full_join(., cazyme_categories) %>%
  dplyr::select(Bin, HMMER, functional_category)


heatmap_cazyme <- cazymes_methanocatella_table2 %>%
...
```{r}
nimI <- fread("./04-analysis/card/methanocatella/methanocatella_card_table.tsv") %>%
...
## Prophage
```{r}
methanocatella_prophages <- fread("../cmc_deep_assembly/methanocatella_prophages.tsv") %>%
...
prophage_heatmap <- ggplot(methanocatella_prophages, aes(x = "Prophage", y = Bin, fill = Prophage)) +
  geom_tile() +
  theme_void() +
  theme(axis.title.x=element_blank(),
        axis.text.x=element_blank(),
...
```{r}
methanogenesis_pres_abs <- fread("05-results/methanocatella_methanogenesis_genes_presence_absence.tsv")
...
## Panaroo
```{r}
heatmap_panaroo_data <- fread("./04-analysis/panaroo/methanocatella/w_new_mag/cluster_4/results_cluster_4/gene_presence_absence.Rtab") %>%
...
gene_abundance <- fread("./04-analysis/panaroo/methanocatella/w_new_mag/cluster_4/results_cluster_4/gene_presence_absence.Rtab") %>%
...
```{r}
methanocatella_full_tree_w_scales <- mag_source_heatmap %>% insert_left(methanocatella_tree_age, width = 6) %>% insert_right(heatmap_age, width = 1) %>% insert_right(continent_heatmap) %>% insert_right(latitude, width = 1) %>% insert_right(longitude, width = 1) %>% insert_right(species_clusters_heatmap) %>% insert_right(completeness, width = 1) %>% insert_right(contigs, width = 1) %>% insert_right(n50, width = 1) %>% insert_right(total_SNPs, width = 1) %>% insert_right(gene_count, width = 1) %>% insert_right(heatmap_crispr, width = 1) %>% insert_right(prophage_heatmap, width = 1) %>% insert_right(heatmap_nimI, width = 1) %>% insert_right(heatmap_cazyme, width = 1) %>% insert_right(gene_counts_bar_plot_w_scales, width = 1)

methanocatella_full_tree_w_scales

#ggsave("./05-results/methanocatella_cluster4_full_tree_w_scales.pdf", plot = methanocatella_full_tree_w_scales, device = "pdf", scale = 1, width = 25, height = 7, units = c("in"), dpi = 300) 
```
