...
# Methanocatella Cas III-A

## Nucleotide - MAFFT
```{r}
reference <- "Methanobrevibacter_ruminantium - crispr_IIIA - cas_IIIA"

IIIA_mafft_matrix <- fread("./05-results/methanocatella_casIIIA_MAFFT_matrix.csv") %>%
  rename(sample = 1) %>%
...
IIIA_structures <- fread("05-results/methanocatella_cas_IIIA_nodes_structures.csv") %>%
  rename(Node = 'Node_') %>%
  separate("Node", into = "Node", sep = "\\ -", extra = "drop") %>%
  mutate(Structure = as.character(Structure)) %>%
  rename(Contig = Node) %>%
  full_join(., methanocatella_cas, by = "Contig")
...
## Protein - MUSCLE
```{r}
IIIA_muscle_matrix <- fread("05-results/methanocatella_casIIIA_MUSCLE_matrix.csv") %>%
  rename(sample = 1) %>%
  pivot_longer(-sample, names_to = "sample2", values_to = "value") %>%
  mutate(
    sample  = factor(sample,  levels = c(reference, setdiff(unique(sample),  reference))),
