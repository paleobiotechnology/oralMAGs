---
title: "Methanocatella full tree and metadata"
author: "Keri Burge"
...
methanocatella_tree_age <- methanocatella_tree %<+% metadata + 
...
```{r}
methanocatella_metadata2 <- metadata %>%
  ...
  dplyr::select(Label, Industrialization, primary_cluster, Continent) %>%
  ...
  dplyr::select(Label, primary_cluster, Industrialization, Continent) %>%
  pivot_longer(!Label, names_to = "category", values_to = "Geo_Cluster") %>%
...
heatmap_metadata <- ggplot(methanocatella_metadata2, aes(x = category, y = Label, fill = Geo_Cluster)) +
  geom_tile() +
  theme_void() +
...
#Species cluster
species_clusters_heatmap <- ggplot(metadata, aes(x = "primary_cluster", y = Label, fill = primary_cluster)) +
  geom_tile() +
  theme_void() +
  theme(axis.title.x=element_blank(),
        axis.text.x=element_blank(),
...
#Continent
continent_heatmap <- ggplot(metadata, aes(x = "Continent", y = Label, fill = Continent)) +
  geom_tile() +
  theme_void() +
  theme(axis.title.x=element_blank(),
        axis.text.x=element_blank(),
...
mag_source_heatmap <- metadata %>%
  ...
  mutate(Industrialization = str_replace_all(Industrialization, "Non_industrial", "Modern non-industrial"),
         Industrialization = str_replace_all(Industrialization, "Industrial", "Modern industrial"),
         Industrialization = str_replace_all(Industrialization, "Ancient", "Ancient Human")) %>%
  ggplot(., aes(x = "Industrialization", y = Label, fill = Industrialization)) +
  geom_tile() +
  theme_void() +
  theme(axis.title.x=element_blank(),
        axis.text.x=element_blank(),
...
### Age
```{r}
heatmap_age <- ggplot(metadata, aes(x = "log_age", y = Label, fill = log_age)) +
  geom_tile() +
  theme_void() +
  theme(axis.title.x=element_blank(),
        axis.text.x=element_blank(),
...
### Latitude
```{r}
latitude <- ggplot(metadata, aes(x = "latitude", y = Label, fill = latitude)) +
  geom_tile() +
  theme_void() +
  theme(axis.title.x=element_blank(),
        axis.text.x=element_blank(),
...
### Longitude
```{r}
longitude <- ggplot(metadata, aes(x = "longitude", y = Label, fill = longitude)) +
  geom_tile() +
  theme_void() +
  theme(axis.title.x=element_blank(),
        axis.text.x=element_blank(),
...
### Contig Count
```{r}
contigs <- ggplot(metadata, aes(x = "quast_n_contigs", y = Label, fill = quast_n_contigs)) +
  geom_tile() +
  theme_void() +
  theme(axis.title.x=element_blank(),
        axis.text.x=element_blank(),
...
n50 <- metadata %>%
  ...
  ggplot(., aes(x = "n50", y = Label, fill = n50)) +
  geom_tile() +
  theme_void() +
  theme(axis.title.x=element_blank(),
        axis.text.x=element_blank(),
...
### Completeness
```{r}
completeness <- ggplot(metadata, aes(x = "checkM.completeness", y = Label, fill = checkM.completeness)) +
  geom_tile() +
  theme_void() +
  theme(axis.title.x=element_blank(),
        axis.text.x=element_blank(),
...
oralis <- fread("05-results/methanocatella_oralis_cctyper_cas_table.tsv") 

methanocatella_cas <- fread("05-results/methanocatella_cctyper_cas_table.tsv") %>%
  full_join(., oralis) %>%
...
methanocatella_cas_input <- methanocatella_cas_input %>%
  inner_join(., metadata %>%
               select(Label, Bin), by = "Bin")

heatmap_crispr <- ggplot(methanocatella_cas_input, aes(x = category, y = Label, fill = crispr_metadata)) +
  geom_tile() +
  theme_void() +
  theme(axis.title.x = element_blank(),
        axis.text.x = element_blank(),
...
cazymes_oralis <- fread("05-results/dbcan3_cazyme_table_methanocatella_oralis.tsv")

methanocatella_metadata <- fread("05-results/methanocatella_metadata.tsv") %>%
...
heatmap_cazyme <- cazymes_methanocatella_table2 %>%
  ...
  complete(HMMER, Label, fill = list(functional_category = NA)) %>%
  filter(!is.na(HMMER)) %>% 
  mutate(HMMER = factor(HMMER, levels = c("GT2", "GT4", "GT66", "GT81", "GT116", "GH154"))) %>%
  ggplot(aes(x = HMMER, y = Label, fill = functional_category)) +
...
```{r}
nimI <- fread("05-results/methanocatella_card_table.tsv") %>%
...
heatmap_nimI <- ggplot(nimI, aes(x = "nimI", y = Label, fill = nimI)) +
  geom_tile() +
  theme_void() +
  theme(axis.title.x=element_blank(),
        axis.text.x=element_blank(),
...
## Prophage
```{r}
methanocatella_prophages <- fread("05-results/methanocatella_prophages.tsv") %>%
...
prophage_heatmap <- ggplot(methanocatella_prophages, aes(x = "Prophage", y = Label, fill = Prophage)) +
  geom_tile() +
  theme_void() +
  theme(axis.title.x=element_blank(),
        axis.text.x=element_blank(),
...
gene_counts <- methanogenesis_pres_abs %>%
  ...
  full_join(., metadata %>%
              select(Bin, Label)) %>%
  filter(Bin != "GCF_912073625.1_PRJEB46774_genomic")

gene_counts_bar_plot <- gene_counts %>%
  ggplot(., aes(x = total, y = reorder(Label, total))) +
...
## Panaroo
```{r}
heatmap_panaroo_data <- fread("05-results/methanocatella_full_tree_panaroo_gene_presence_absence.Rtab") %>%
...
gene_abundance <- fread("05-results/methanocatella_full_tree_panaroo_gene_presence_absence.Rtab") %>%
...
individual_id_species <- individual_id_species[c("1187", "1188", "1189", "1190"), , drop = FALSE]
...
comb_order_from_labels <- function(comb_mat, set_order, desired_labels, append_rest = TRUE) {
  ...
  labels <- vapply(cn, function(x) {
    ...
    if (length(inc) == 0) "∅" else paste(inc, collapse = "&")
  }, character(1))
...
set_order <- c("1187","1188","1189","1190")
interaction_order <- c("1187", "1188&1187", "1189&1187", "1187&1190", "1188", "1188&1190", "1189", "1189&1190", "1190")
