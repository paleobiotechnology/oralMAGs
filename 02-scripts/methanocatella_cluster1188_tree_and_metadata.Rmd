---
title: "Methanocatella oralis (pr_cl_1188) Tree and Metadata"
author: "Keri Burge"
...
```{r}
tre_methanocatella_c1188_scc <- ape::read.tree("./05-results/methanocatella_c1188_scc.raxml.support")

methanocatella_c1188_mad_root <- mad(tre_methanocatella_c1188_scc, 'newick')

methanocatella_c1188_mad_root %>%
  as.list() %>%
  fwrite(., "./05-results/methanocatella_c1188_mad_root.nwk", quote = F)
```

```{r}
# Find the root
methanocatella_c1188_mad_root <- mad(tre_methanocatella_c1188_scc, 'full')

#ERS11702757-megahit_001 is the root
```


```{r}
methanocatella_c1188_mad_root <- ape::read.tree("./05-results/methanocatella_c1188_scc.raxml.support")

methanocatella_c1188_mad_root <- root(methanocatella_c1188_mad_root, outgroup = "ERS11702757-megahit_001")

methanocatella_c1188_mad_root$tip.label <- methanocatella_c1188_mad_root$tip.label %>%
  as.data.frame() %>%
  dplyr::rename(Bin = 1) %>%
...
# create tree
methanocatella_tree_c1188 <- ggtree(methanocatella_c1188_mad_root, ladderize = T)
...
methanocatella_tree_c1188_age <- methanocatella_tree_c1188 %<+% metadata + 
...
```{r}
#fastBAPS cluster
fastBAPS <- fread("05-results/fastBAPS_methanocatella_cluster1188_snp_counts_and_clusters.tsv") %>%
  mutate(Bin = if_else(str_detect(Bin, "GC|refbinned"), Bin, 
                        str_replace_all(Bin, "-metaspades_00", ".bin.r.") %>%
                        str_replace_all("-metaspades_0", ".bin.r.") %>%
                        str_replace_all("-metaspades", ".bin.r.") %>%
...
heatmap_age <- ggplot(metadata, aes(x = "log_age", y = Bin, fill = log_age)) +
  ...
  theme(axis.title.x=element_blank(),
        ...
        axis.text.y=element_blank(),
        axis.ticks.y=element_blank()) +
  scale_fill_viridis_c(option = "mako", name = "Age (BP) (log10)", direction = -1, limits = c(0.845098, 5.008600))
...
### Gene Count
```{r}
scc_gene_list <- fread("05-results/methanocatella_C1188_non_recomb_scc_gene_list.tsv") %>%
  pull()

panaroo <- fread("05-results/methanocatella_c1188_panaroo_gene_presence_absence.Rtab") %>%
...
heatmap_crispr <- ggplot(methanocatella_cas_input, aes(x = category, y = Bin, fill = crispr_metadata)) +
  ...
  theme(axis.title.x = element_blank(),
        ...
        axis.ticks.y = element_blank()) +
  scale_fill_manual(
    values = c(new_crispr_colors, "#f0f0f0"), 
    na.value = "#f0f0f0" 
  )
...
##Panaroo
```{r}
heatmap_panaroo_data <- fread("05-results/methanocatella_c1188_panaroo_gene_presence_absence.Rtab") %>%
...
gene_abundance <- fread("05-results/methanocatella_c1188_panaroo_gene_presence_absence.Rtab") %>%
