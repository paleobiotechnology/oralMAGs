...
number_mags_plot <- stats %>%
  ...
  mutate(Genera = factor(Genera, levels = c("Methanomethylophilus", "Methanocatella"))) %>%
...
number_mags_plot_methanocatella <- stats %>%
  dplyr::select(Bin, Genera, Host, pass.GUNC, checkM.completeness, checkM.contamination) %>%
  filter(Genera == "Methanocatella") %>%
...
number_mags_plot_methanocatella

#ggsave("./05-results/methanocatella_number_mags_by_host_and_quality_w_CSC.pdf", plot = number_mags_plot_methanocatella, device = "pdf", scale = 1, width = 8, height = 4, units = c("in"), dpi = 300) 
...
```
# B. Number of contigs per MAG
Species across the top and genera rows

```{r}
combined_metadata <- methanocatella_metadata %>%
  full_join(., methanomethylophilus_metadata) %>%
  rename(Genus = Genera) %>%
  filter(!str_detect(Bin, "GC"))
```
...
n_contigs_plot <- combined_metadata %>%
  ...
  mutate(Genus = factor(Genus, levels = c("Methanomethylophilus", "Methanocatella"))) %>%
...
#C. Damage estimates for reads of each sample mapped back to contigs in the RGIG7111 pr_cl_1286 MAG produced from that sample. 
...
```{r}
methanocatella_damage_curves <- fread("./05-results/methanocatella_damage_curves.tsv")

methanocatella_summary_counts <- methanocatella_damage_curves %>%
...
methanocatella_damage_curves_plot <- methanocatella_damage_curves %>%
  ggplot(., aes(x = Base, y = mean_damage, group = Bin)) + # , group = Country
         ...
         geom_text(data = methanocatella_summary_counts,
                          aes(x = Base, y = mean_damage, label = label),
                          inherit.aes = FALSE,
                          size = 3.5)

methanocatella_damage_curves_plot
...
```{r}
methanomethylophilus_damage_curves <- fread("./05-results/methanomethylophilus_damage_curves.tsv")
...
methanocatella_panaroo <- fread("05-results/methanocatella_full_tree_panaroo_gene_presence_absence.Rtab") %>%
  pivot_longer(-Gene, names_to = "Sample", values_to = "pres_abs") %>%
  group_by(Sample) %>%
  mutate(pres_abs = as.integer(pres_abs)) %>%
  summarize(count = sum(pres_abs)) %>%
...
gene_count <- methanomethylophilus_panaroo %>%
  rename(prepublication_bin = Bin) %>%
  full_join(., methanocatella_panaroo %>%
              rename(prepublication_bin = Bin))
...
gene_count_v_completeness <- combined_metadata %>%
  ...
  mutate(Genus = ifelse(str_detect(Bin, "GCF_001639275.1_ASM163927v1_genomic"), "Methanocatella", Genus)) %>%
  ...
  mutate(mag_source = str_replace_all(mag_source, "Non_industrial", "CMC"),
         mag_source = str_replace_all(mag_source, "Industrial", "Modern")) %>%
  mutate(mag_source = ifelse(str_detect(Host, "Ancient human"), "Ancient human", mag_source),
         mag_source = ifelse(str_detect(sample_host, "neanderthalensis"), "Neanderthal", mag_source),
         mag_source = ifelse(str_detect(Industrialization, "Non_industrial"), "CMC", mag_source)) %>%
